...
if len(hari) != len(tarikh) or len(tarikh) != len(sale):
    ...
    ax.set_xticklabels(label_x, rotation=45, ha="right")  # Show both 'hari' and 'tarikh'
    for i, txt in enumerate(sale):
        ax.text(i, txt + 5, str(txt), ha='center', fontsize=8)

    # Tambah garis purata jualan
    ax.axhline(y=purata_sale, color='blue', linestyle='--', linewidth=1.5, label=f'Purata: RM {purata_sale:.2f}')
    ax.legend()

    # Menambah jumlah dan purata jualan di bawah graf
